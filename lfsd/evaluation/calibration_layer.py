...
from lfsd.dataloader import build_detection_test_loader
from lfsd.evaluation.archs import resnet101
    ...
    def execute_calibration(self, inputs, dts):
        ...
        for i in range(ileft, iright):
            ...
            if tmp_class in self.exclude_cls:
                continue
            if tmp_class not in self.prototypes:
                continue
            tmp_cos = cosine_similarity(features[i - ileft].cpu().data.numpy().reshape((1, -1)),
                                        self.prototypes[tmp_class].cpu().data.numpy())[0][0]
            tmp_cos_tensor = torch.tensor(float(tmp_cos), dtype=torch.float32, device=dts[0]['instances'].scores.device)
            dts[0]['instances'].scores[i] = dts[0]['instances'].scores[i] * self.alpha + tmp_cos_tensor * (1 - self.alpha)
        return dts
