...
from lfsd.evaluation.evaluator import DatasetEvaluator
...
class COCOEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, distributed, output_dir=None):
        ...
        # Build reverse mapping: contiguous_id -> dataset_id
        ...
        for input, output in zip(inputs, outputs):
            ...
            if "instances" in output:
                ...
                prediction["instances"] = instances_to_coco_json(
                    instances, input["image_id"], self._contiguous_id_to_dataset_id)
    ...
    def _eval_predictions(self):
        ...
        # NOTE: category_id mapping is already done in instances_to_coco_json
...
def instances_to_coco_json(instances, img_id, contiguous_id_to_dataset_id=None):
    """
    ...
    Args:
        ...
        contiguous_id_to_dataset_id (dict): mapping from contiguous id to dataset id
    ...
    for k in range(num_instance):
        # Convert contiguous id back to dataset id if mapping is provided
        cat_id = classes[k]
        if contiguous_id_to_dataset_id is not None:
            cat_id = contiguous_id_to_dataset_id.get(cat_id, cat_id)
        result = {
            "image_id": img_id,
            "category_id": cat_id,
            "bbox": boxes[k],
