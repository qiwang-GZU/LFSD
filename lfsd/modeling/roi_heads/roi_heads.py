...
class CommonalityROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.queue_enabled = self.memory or self.semantic or self.augmentation
        if (self.semantic or self.augmentation) and not self.memory and self.queue_enabled:
            logger.warning(
                "MODEL.ROI_HEADS.MEMORY is False, but semantic/augmentation heads "
                "require the prototype queues. Enabling queues automatically."
            )
        ...
        dataset_name = self.cfg.DATASETS.TRAIN[0].lower()
        if 'voc' in dataset_name:
            self.novel_index = [15, 16, 17, 18, 19]
        elif 'coco' in dataset_name:
            self.novel_index = [0, 1, 2, 3, 4, 5, 6, 8, 14, 15, 16, 17, 18, 19, 39, 56, 57, 58, 60, 62]
        elif 'bcgd' in dataset_name:
            self.novel_index = list(range(self.num_classes))
        else:
            logger.warning(
                "Unable to infer novel classes for dataset '%s'. "
                "Defaulting to treat every class as novel.", dataset_name
            )
            self.novel_index = list(range(self.num_classes))
        self.base_index = [i for i in range(self.num_classes) if i not in self.novel_index]
        if len(self.base_index) == 0:
            logger.warning(
                "No base classes detected for dataset '%s'; "
                "falling back to reuse all %d classes as base references.",
                dataset_name,
                self.num_classes,
            )
            self.base_index = list(range(self.num_classes))
    ...
    def predict_prototype(self, feature_pooled_s, feature_pooled_l, gt_classes):
        """
        Predict class logits using prototype matching.
        Concise implementation consistent with LFSD-EAEC.
        """
        prototypes_s = []
        prototypes_l = []
        
        for i in range(self.num_classes):
            # Consistent with LFSD-EAEC: always use mean regardless of queue status
            # This avoids cosine similarity issues caused by zero vectors
        ...
        # Use F.cosine_similarity (consistent with LFSD-EAEC)
    ...
    def generate_features(self, gt_classes):
        ...
        for c in self.base_index:
            if self.queue_full[c]:
                feats_s = self.queue_s[c]
            else:
                ptr = int(self.queue_ptr[c])
                if ptr == 0:
                    continue
                feats_s = self.queue_s[c][:ptr]

            c_mean = feats_s.mean(dim=0)
            c_std = feats_s.var(dim=0, unbiased=False)
            if self.queue_full[c]:
                if self.queue_len > 1:
                    c_std = c_std * self.queue_len / (self.queue_len - 1)
            elif feats_s.size(0) > 1:
                c_std = c_std * feats_s.size(0) / (feats_s.size(0) - 1)
        ...
        for c in self.novel_index:
            ...
            calibrated_std = torch.clamp(calibrated_std, min=1e-6)
        ...
        if self.training:          
            ...
            if int(storage.iter) >= self.warmup_distill:
                ...
                if self.semantic:
                    ...
                    # Concise implementation consistent with LFSD-EAEC, directly compute KL divergence
                    loss_kld = F.kl_div(
                        F.log_softmax(pred_class_logits[true_cases], dim=1),
                        gt_prototype_classes_s[true_cases],
                        reduction='batchmean'
                    )
    ...
    def gaussian_kernel(self,x, y, sigma):
        dist = torch.sum(x ** 2, dim=1, keepdim=True) + torch.sum(y ** 2, dim=1, keepdim=True).T - 2 * torch.mm(x, y.T)
