...
class FastRCNNOutputs(object):
    ...
    def smooth_l1_loss_distill(self, gt_prototype_classes):
        ...
        # Handle empty foreground case
        if fg_inds.numel() == 0:
            return self.pred_proposal_deltas.sum() * 0.0
...
class DoubleFastRCNNOutputLayers(nn.Module):
    def __init__(
        ...
        weight = torch.sqrt(torch.tensor(num_class_prac / (num_class_prac - 1))) * (
                    torch.eye(num_class_prac) - (1 / num_class_prac) * torch.ones((num_class_prac, num_class_prac)))
        ...
        # self.bbox_pred = nn.Linear(input_size, input_size)
        # self.bbox_pred = nn.ReLU(inplace=True)
        # self.bbox_pred = nn.Linear(input_size, num_bbox_reg_classes * box_dim)
        # self.bbox_pred= nn.Sequential(
        #     nn.Linear(input_size, input_size),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(input_size, num_bbox_reg_classes * box_dim)
        # )
        weight_init.c2_xavier_fill(self.bbox_pred)
        nn.init.normal_(self.bbox_pred.weight, std=0.001)
        ...
        # Consistent with LFSD-EAEC: freeze cls_score
        self.cls_score.requires_grad_(False)
        self.etf_residual = True   
        logger.info("ETF Residual {}".format(self.etf_residual))
    ...
    def forward(self, x_s, x_l):
        ...
        x_s = self.mapping(x_s) + x_s if self.etf_residual else self.mapping(x_s)
        scores = self.cls_score(x_s)
